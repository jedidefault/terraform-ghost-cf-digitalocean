...
resource "digitalocean_droplet" "ghost-18-04" {
  image  = var.digitalocean_droplet_image
  name   = var.digitalocean_droplet_name
  region = var.digitalocean_droplet_region
  size   = var.digitalocean_droplet_size
  ssh_keys = [data.digitalocean_ssh_key.ewok_ssh_key.id]
  backups = var.digitalocean_droplet_backup
}

data "digitalocean_droplet" "ghost-18-04" {
  name = var.digitalocean_droplet_name
  depends_on = [digitalocean_droplet.ghost-18-04]
